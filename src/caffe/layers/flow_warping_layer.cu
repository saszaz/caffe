...
#include "caffe/util/ekf_math.hpp"
...
__global__ void FlowWarpingBilinearCoefs(const int nthreads, Dtype* bi_coefs, Dtype* partial_i, Dtype*
        ...
        const Dtype* disp, const Dtype disp_scale) {
    CUDA_KERNEL_LOOP(index, nthreads) {
        ...
        Dtype y = disp_scale * disp[index] + i;
        Dtype x = disp_scale * disp[w * h + index] + j;
        int r = (int) y;
        int c = (int) x;
        Dtype rem_x = x - c;
        Dtype rem_y = y - r;
  ...
  for(int i = 0; i < top[0]->num(); i++) {
      ...
      FlowWarpingBilinearCoefs<Dtype><<<CAFFE_GET_BLOCKS(count),
          CAFFE_CUDA_NUM_THREADS>>>(count, interp_coefs, partial_i, partial_j,
                  indices, ptrs, width_, height_, disp, scale_);
...
void FlowWarpingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  const Dtype* input_im = bottom[0]->gpu_data();
  Dtype* scale_vector = scale_blob_.mutable_gpu_data();
  ...
  caffe_gpu_set(channels_, scale_, scale_vector);
  for(int i = 0; i < top[0]->num(); i++) {
      if(propagate_down[1]) {
        ...
        caffe_gpu_gemm(CblasTrans, CblasNoTrans, count, 1, channels_,
        		(Dtype) 1.0, im_diff, scale_vector, (Dtype) 0.0, disp_diff);
